...
byte eeprom_data[14], start, data;
...
void setup(){
  ...
  Wire.begin();
  startWifi();  //Start the wire library as master
  ...
  for(data = 0; data <= 13; data++)eeprom_data[data] = EEPROM.read(data);               //Read EEPROM for faster data access

  gyro_address = eeprom_data[10];                                                       //Store the gyro address in the variable.

  set_gyro_registers();                                                                 //Set the specific gyro registers.

  //Check the EEPROM signature to make sure that the setup program is executed.
  while(eeprom_data[11] != 'J' || eeprom_data[12] != 'M' || eeprom_data[13] != 'B'){
...
void loop(){
  ...
  if(lastRecPacketThrottle < 1025)new_function_request = false;                      //If the throttle is in the lowest position set the request flag to false.
  ...
  if(data == 0 && new_function_request == false){                                       //Only start the calibration mode at first start. 
    esc_1 = lastRecPacketThrottle;                                                   //Set the pulse for motor 1 equal to the throttle channel.
    esc_2 = lastRecPacketThrottle;                                                   //Set the pulse for motor 2 equal to the throttle channel.
    esc_3 = lastRecPacketThrottle;                                                   //Set the pulse for motor 3 equal to the throttle channel.
    esc_4 = lastRecPacketThrottle;                                                   //Set the pulse for motor 4 equal to the throttle channel.
    esc_pulse_output();                                                                 //Send the ESC control pulses.
  }
  ...
  if(data == 'r'){
    ...
    if(lastRecPacketThrottle < 1050 && lastRecPacketYaw < 1050)start = 1;
    //When yaw stick is back in the center position start the motors (step 2).
    if(start == 1 && lastRecPacketThrottle < 1050 && lastRecPacketYaw > 1450)start = 2;
    //Stopping the motors: throttle low and yaw right.
    if(start == 2 && lastRecPacketThrottle < 1050 && lastRecPacketYaw > 1950)start = 0;
  ...
  if(data == '1' || data == '2' || data == '3' || data == '4' || data == '5'){          //If motor 1, 2, 3 or 4 is selected by the user.
    ...
    if(new_function_request == true && loop_counter == 250){                            //Wait for the throttle to be set to 0.
      ...
      Serial.println(lastRecPacketThrottle);                                         //Print the actual throttle position.
      loop_counter = 0;                                                                 //Reset the loop_counter variable.
    }
    if(new_function_request == false){                                                  //When the throttle was in the lowest position do this.
      if(data == '1' || data == '5')esc_1 = lastRecPacketThrottle;                   //If motor 1 is requested set the pulse for motor 1 equal to the throttle channel.
      else esc_1 = 1000;                                                                //If motor 1 is not requested set the pulse for the ESC to 1000us (off).
      if(data == '2' || data == '5')esc_2 = lastRecPacketThrottle;                   //If motor 2 is requested set the pulse for motor 1 equal to the throttle channel.
      else esc_2 = 1000;                                                                //If motor 2 is not requested set the pulse for the ESC to 1000us (off).
      if(data == '3' || data == '5')esc_3 = lastRecPacketThrottle;                   //If motor 3 is requested set the pulse for motor 1 equal to the throttle channel.
      else esc_3 = 1000;                                                                //If motor 3 is not requested set the pulse for the ESC to 1000us (off).
      if(data == '4' || data == '5')esc_4 = lastRecPacketThrottle;                   //If motor 4 is requested set the pulse for motor 1 equal to the throttle channel.
      ...
      if(eeprom_data[9] == 1){                                                         //The MPU-6050 is installed
...
void print_signals(){
  ...
  Serial.print("  Roll:");
  if(lastRecPacketRoll - 1480 < 0)Serial.print("<<<");
  else if(lastRecPacketRoll - 1520 > 0)Serial.print(">>>");
  else Serial.print("-+-");
  Serial.print(lastRecPacketRoll);

  Serial.print("  Pitch:");
  if(lastRecPacketPitch - 1480 < 0)Serial.print("^^^");
  else if(lastRecPacketPitch - 1520 > 0)Serial.print("vvv");
  else Serial.print("-+-");
  Serial.print(lastRecPacketPitch);

  Serial.print("  Throttle:");
  if(lastRecPacketThrottle - 1480 < 0)Serial.print("vvv");
  else if(lastRecPacketThrottle - 1520 > 0)Serial.print("^^^");
  else Serial.print("-+-");
  Serial.print(lastRecPacketThrottle);

  Serial.print("  Yaw:");
  if(lastRecPacketYaw - 1480 < 0)Serial.print("<<<");
  else if(lastRecPacketYaw - 1520 > 0)Serial.print(">>>");
  else Serial.print("-+-");
  Serial.println(lastRecPacketYaw);
}
...
void gyro_signalen(){
  ...
  gyro_roll = gyro_axis[eeprom_data[6] & 0b00000011];           //Set gyro_roll to the correct axis that was stored in the EEPROM.
  if(eeprom_data[6] & 0b10000000)gyro_roll *= -1;               //Invert gyro_roll if the MSB of EEPROM bit 28 is set.
  gyro_pitch = gyro_axis[eeprom_data[7] & 0b00000011];          //Set gyro_pitch to the correct axis that was stored in the EEPROM.
  if(eeprom_data[7] & 0b10000000)gyro_pitch *= -1;              //Invert gyro_pitch if the MSB of EEPROM bit 29 is set.
  gyro_yaw = gyro_axis[eeprom_data[8] & 0b00000011];            //Set gyro_yaw to the correct axis that was stored in the EEPROM.
  if(eeprom_data[8] & 0b10000000)gyro_yaw *= -1;                //Invert gyro_yaw if the MSB of EEPROM bit 30 is set.

  acc_x = acc_axis[eeprom_data[7] & 0b00000011];                //Set acc_x to the correct axis that was stored in the EEPROM.
  if(eeprom_data[7] & 0b10000000)acc_x *= -1;                   //Invert acc_x if the MSB of EEPROM bit 29 is set.
  acc_y = acc_axis[eeprom_data[6] & 0b00000011];                //Set acc_y to the correct axis that was stored in the EEPROM.
  if(eeprom_data[6] & 0b10000000)acc_y *= -1;                   //Invert acc_y if the MSB of EEPROM bit 28 is set.
  acc_z = acc_axis[eeprom_data[8] & 0b00000011];                //Set acc_z to the correct axis that was stored in the EEPROM.
  if(eeprom_data[8] & 0b10000000)acc_z *= -1;                   //Invert acc_z if the MSB of EEPROM bit 30 is set.
}
